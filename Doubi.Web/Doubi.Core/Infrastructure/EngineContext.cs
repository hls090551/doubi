        ...
        public static IEngine CreateEngineInstance(SmartStoreConfig config)
        {
            if (config != null && !string.IsNullOrEmpty(config.EngineType))
            {
                ...
                if (!typeof(IEngine).IsAssignableFrom(engineType))
                    throw new ConfigurationErrorsException("The type '" + engineType + "' doesn't implement 'Doubi.Core.Infrastructure.IEngine' and cannot be configured in /configuration/SmartStoreConfig/engine[@engineType] for that purpose.");
